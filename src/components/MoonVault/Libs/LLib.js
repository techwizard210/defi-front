...
export const LLib = {
  ...
  formatDecimal(_value, _minDecimals = 2, _maxDecimals = 18) {
  ...
  defaultUndefined: (_value, _default) => {
  ...
  smartFormatPercent(_value, _shorten, _minDecimals, _maxDecimals) {
    if (_value > 999) {
      return '> 999%';
    }
    ...
    const val = this.formatDecimal(
      _value,
      _minDecimals ?? minDecimals,
      _maxDecimals ?? 2
    );
  ...
  smartFormatFloat(_value, _shorten) {
  ...
  smartFormatFloatDisplay(_value, _shorten, _minDecimals, _maxDecimals) {
  ...
  smartFormatMultiplicator(_value, _shorten) {
  ...
  getVersion(_version) {
    const s = _version.split('.');
    return {
      major: parseInt(s[0], 10),
      minor: parseInt(s.length >= 2 ? s[1] : 0, 10),
      revision: parseInt(s.length >= 3 ? s[2] : 0, 10),
    };
  },
  ...
  getDateTimeFromDB(_string) {
    ...
    return new Date(parseInt(_string, 10) * 1000);
  },

  async measureTime(_topic, _action, debug) {
    await _action();
  ...
  checkRequiredVersion(_version, _major, _minor, _revision) {
    ...
    if (
      ...
      (_version.major === _major && _version.minor > _minor) ||
      (_version.major === _major &&
  ...
  removeTrail(_string, _remove, _start) {
    ...
    if (_start) {
      ...
      for (let n = _string.length - 1; n >= 0; n -= 1) {
        if (_string[n] !== _remove) {
          break;
        }
        o += 1;
    ...
    return _start
      ? _string.substring(o)
      : _string.substring(0, _string.length - o);
  },

  getHMS(_seconds, _showHMS) {
    let ret = '';

    // seconds
    const s = parseInt(_seconds, 10) % 60;
    // eslint-disable-next-line no-param-reassign
    _seconds /= 60;

    // minutes
    const m = parseInt(_seconds, 10) % 60;
    // eslint-disable-next-line no-param-reassign
    _seconds /= 60;

    // hours
    const h = parseInt(_seconds, 10);
    ...
    if (m > 0) {
      ...
      ret +=
        m.toString().padStart(ret !== '' && !_showHMS ? 2 : 1, '0') +
        (_showHMS ? 'm' : '');
    }
    if (s >= 0) {
      if (ret !== '') {
      ...
      ret +=
        s.toString().padStart(ret !== '' && !_showHMS ? 2 : 1, '0') +
        (_showHMS ? 's' : '');
    }
  ...
  getStorage_bool(_sessionOnly, _name, _defaultValue) {
    const val = this.getStorage(_sessionOnly, _name, _defaultValue);
    return val === _defaultValue ? val : val === 'true';
  },

  getStorage_int(_sessionOnly, _name, _defaultValue) {
    const val = this.getStorage(_sessionOnly, _name, _defaultValue);
    return val === _defaultValue ? val : parseInt(val, 10);
  },

  getStorage_float(_sessionOnly, _name, _defaultValue) {
    const val = this.getStorage(_sessionOnly, _name, _defaultValue);
    return val === _defaultValue ? val : parseFloat(val);
  },

  formatFiat(_amount, _useRegional = true, _smartFormat) {
    // eslint-disable-next-line no-param-reassign
    _amount = _amount ?? 0;
  ...
  formatDate(_date, _includeTime = false, _includeSeconds = false) {
    const baseFormat = this.getLocaleDateString();

    let formatString = baseFormat;
    if (_includeTime) {
      formatString = _includeSeconds ? `${baseFormat} LTS` : `${baseFormat} LT`;
    }

    return moment(_date).format(formatString);
  },

  formatTime(_date, _includeSeconds = false) {
