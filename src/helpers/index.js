...
export function ddhhmmss(secs) {
  ...
  return `${pad(days)}:${pad(hours % 24)}:${pad(minutes % 60)}:${pad(
    secs % 60
  )}`;
}
 ...
 * @returns {string}
 */
export const parseBalance = (balance, decimalsToDisplay = 3) =>
  Number(formatUnits(balance)).toFixed(decimalsToDisplay);
...
export const getGameFactoryContractInstance = () => {
  if (_isWeb3Defined()) {
    const newFactoryContract = new web3.eth.Contract(
      FactoryABI,
      FactoryAddress
    );
...
export const getUserBoostExpiryTimestamp = async (userAddress, boostItemId) => {
  ...
  if (gameFactory && userAddress) {
    const expiryBlock = await gameFactory.methods
      .boostItemExpireTime(userAddress, boostItemId)
      .call();
    return expiryBlock;
  }
  return 0;
...
export const getUserStakingAmount = async () => {
  ...
  const userAddress = await getDefaultAddres();
  if (gameFactory && userAddress) {
    const staked = await gameFactory.methods
      .getStakedAmountByUser(userAddress)
      .call();
    ...
    return [
      parseBalance(staked[0]),
      parseBalance(staked[1]),
      bronzeItemStatus.activeStatus ? 1 : 0,
      silverItemStatus.activeStatus ? 1 : 0,
      goldItemStatus.activeStatus ? 1 : 0,
    ];
  }
  return [0, 0, 0, 0, 0];
};
...
export const getGameStatisticData = async () => {
  ...
  if (gameFactory && userAddress) {
    ...
    return {
      ...
      totalFrozen:
        Number(parseBalance(statisticData.totalStakedForPlay)) +
        Number(parseBalance(statisticData.totalStakedForEarn)) +
        Number(parseBalance(statisticData.totalStakedForBoost)),
...
export const getThawingPeriod = async () => {
  ...
  const userAddress = await getDefaultAddres();
  if (gameFactory && userAddress) {
    const thawingPeriod = await gameFactory.methods
...
export const getUserThawing = async () => {
  ...
  const userAddress = await getDefaultAddres();
  if (gameFactory && userAddress) {
    const userThawingData = await gameFactory.methods
      .getThawingStateByUser()
      .call({ from: userAddress });
    return {
      approvedAmount: web3.utils.fromWei(
        userThawingData.approvedAmount,
        'ether'
      ),
...
export const getUserReward = async () => {
  ...
  const userAddress = await getDefaultAddres();
  if (gameFactory && userAddress) {
    const userRewardData = await gameFactory.methods
      .getRewardStateByUser()
      .call({ from: userAddress });
    return {
      approvedAmount: web3.utils.fromWei(
        userRewardData.approvedAmount,
        'ether'
      ),
...
export const approveTokenToContract = async (balance, userAddress) => {
  const calculatedApproveValue = parseUnits(
...
export const displayFormatTime = (time) => {
  ...
  const hours =
    Math.floor(Number(tValue / 3600)) > 9
      ? Math.floor(Number(tValue / 3600))
      : `0${Math.floor(Number(tValue / 3600))}`;
  const minutes =
    Math.floor(Number(tValue % 3600) / 60) > 9
      ? Math.floor(Number(tValue % 3600) / 60)
      : `0${Math.floor(Number(tValue % 3600) / 60)}`;
  const seconds =
    Number(tValue % 60).toFixed(0) > 9
      ? Number(tValue % 60).toFixed(0)
      : `0${Number(tValue % 60).toFixed(0)}`;
...
export const approveTokenToNFTStakeContract = async (balance, userAddress) => {
  ...
  const tokenContract = getTokenContractInstance();
  const allowance = await tokenContract.methods
    .allowance(userAddress, NFTStakingAddress)
    .call();
  if (allowance < balance) {
    await tokenContract.methods
      .approve(NFTStakingAddress, calculatedApproveValue)
      .send({ from: userAddress });
  }
};
